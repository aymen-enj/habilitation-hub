...
export default function AccessRequests() {
  const { session, agents, applications, profiles, modules, requests, users, createRequest } = useDemo();
  ...
  const visibleRequests = useMemo(() => {
    ...
    if (role === "MANAGER") return requests.filter((r) => r.requesterId === session!.user.id || r.requesterId === "u-mgr");
    return requests;
    ...
    <div className="cnss-page space-y-6">
      ...
      {!canCreate && (
        <AlertBanner tone="info" title="Mode consultation">
          Ce rôle est en consultation uniquement. Vous pouvez consulter les demandes mais pas en créer.
        </AlertBanner>
      )}
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        ...
        <section aria-labelledby="form-title" className="cnss-card p-6">
          <h2 id="form-title" className="mb-1 text-lg font-semibold text-foreground">Nouvelle demande</h2>
          <p className="mb-5 text-sm text-muted-foreground">
            Renseignez les informations ci-dessous. Les champs marqués d'une étoile sont obligatoires.
          </p>

          <fieldset disabled={!canCreate} className="contents">
            <form onSubmit={onSubmit} className="space-y-5" noValidate>
              <FormField id="beneficiary" label="Bénéficiaire" required error={errors.beneficiaryId}>
                <Select value={form.beneficiaryId} onValueChange={(v) => update({ beneficiaryId: v })}>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <FormField id="app" label="Application" required error={errors.applicationId}>
                  <Select
                    ...
                    onValueChange={(v) => update({ applicationId: v, profileId: "", moduleId: "" })}
                  >
                ...
                <FormField id="profile" label="Profil" required error={errors.profileId}>
                  ...
                  >
                    <SelectTrigger id="profile">
                      <SelectValue placeholder={form.applicationId ? "Choisir…" : "Application requise"} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <FormField id="module" label="Module" required error={errors.moduleId}>
                  ...
                  >
                    <SelectTrigger id="module">
                      <SelectValue placeholder={form.profileId ? "Choisir…" : "Profil requis"} />
                    </SelectTrigger>
                    <SelectContent>
        ...
        <section aria-labelledby="list-title" className="cnss-card p-6">
          <div className="mb-4 flex items-center justify-between">
            <h2 id="list-title" className="text-lg font-semibold text-foreground">Mes demandes visibles</h2>
            <span className="text-xs text-muted-foreground">{sortedVisible.length} élément(s)</span>
          </div>
          ...
          {sortedVisible.length === 0 ? (
            ...
            <ul className="space-y-3">
              {sortedVisible.map((r) => (
                <li key={r.id} className="rounded-xl border border-border bg-muted/20 p-4 transition-colors hover:bg-muted/40">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Demandé par {requesterName(r.requesterId)} · {formatRelative(r.createdAt)}
                      </p>
                    </div>
                    <StatusBadge status={r.status} />
                  </div>
                  <p className="mt-2 line-clamp-2 text-xs text-muted-foreground">{r.justification}</p>
